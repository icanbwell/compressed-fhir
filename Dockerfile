...
WORKDIR /usr/src/compressedfhir/
...
COPY Pipfile* /usr/src/compressedfhir/
...
ENV PROJECT_DIR=/usr/src/compressedfhir
ENV FLASK_APP=compressedfhir.api
...
COPY ./compressedfhir ${PROJECT_DIR}

# Copy the Pipfile.lock from the previous stage in case it was locked
COPY --from=python_packages /usr/src/compressedfhir/Pipfile.lock /usr/src/compressedfhir/Pipfile.lock

# Copy Pipfile.lock to a temporary directory so it can be retrieved if needed
COPY --from=python_packages /usr/src/compressedfhir/Pipfile.lock /tmp/Pipfile.lock
